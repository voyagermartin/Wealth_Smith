...
| 平均持股成本 | 加權平均買入成本單價 `(歷史買入總成本 / 歷史買入總股數)`，賣出交易不影響每股成本單價，清倉時自動顯示 `$0.00` |
...
| 個股總報酬率 (%) | `[(當前未實現市值 + 個股總股息 + 已實現賣出淨回收金額) - 個股總投入成本] / 個股總投入成本`（解決清倉賣光時舊公式誤算為 -100% 之問題） |
...
### 📅 2026-08-11
* **財務會計邏輯與清倉/買賣報表公式全面驗算校正**：
  * **個股總報酬率 (I欄) 已實現回收校正**：修復舊公式漏算已實現賣出回收金額導致 100% 清倉賣光時報酬率誤算為 `-100%` 之會計問題。全新公式導入 `[(當前未實現市值 + 總股息 + 已實現賣出淨回收金額) - 個股總買入成本] / 個股總買入成本`。
  * **平均持股成本 (E欄) 加權平均單價演算法校正**：修復舊公式以賣出後剩餘股數做分割導致有買有賣時每股成本爆漲之問題。採用嚴謹移動平均成本法 `歷史買入總成本 / 歷史買入總股數`，確保賣出交易不影響剩餘持股單價，100% 清倉時自動歸零 `$0.00`。
  * **XIRR 時間年化效益解析驗證**：完成靜態總報酬率（不計時間）與動態 XIRR（年化時間與現金流加權）之演算法驗算，確認系統數據具備極高會計嚴謹度與準確性。
